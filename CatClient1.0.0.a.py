...
class CatClientv0HDR(tk.Tk):
    ...
    def init_ui(self):
        ...
        launch_button = tk.Button(button_frame, text="PLAY", font=("Arial", 12, "bold"),
                                ...
                                bd=0, padx=20, pady=10, command=self.download_and_launch)
        ...
        news_label = tk.Label(content_area, text="LATEST UPDATES", font=("Arial", 14, "bold"),
                            bg=THEME['bg'], fg=THEME['text'])
        news_label.pack(anchor="w", pady=(0, 20))

        # Placeholder news items
        news_items = [
            "Welcome to CatClient! 🎮",
            "Optimized performance for better FPS 🚀",
            "Added custom skin support 🎨",
            "Improved launcher stability 🛠️"
        ]

        for item in news_items:
            news_item = tk.Frame(content_area, bg=THEME['sidebar'], padx=15, pady=15)
            news_item.pack(fill="x", pady=(0, 10))
            tk.Label(news_item, text=item, font=("Arial", 11),
